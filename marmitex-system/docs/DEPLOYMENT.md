# Guia de Deploy - Marmitex System

## 🚀 Opções de Deploy

Este guia cobre as opções mais populares para fazer deploy do Marmitex System em produção.
...
# Teste a conexão
psql -h seu-rds-endpoint.amazonaws.com -U admin -d marmitex
```
...
# Crie .env
cat > .env << EOF
DATABASE_URL=postgresql://admin:senha@seu-rds-endpoint.amazonaws.com:5432/marmitex
...
sudo tee /etc/nginx/sites-available/marmitex << EOF
server {
    listen 80;
...
# Ative o site
sudo ln -s /etc/nginx/sites-available/marmitex /etc/nginx/sites-enabled/
sudo nginx -t
...
sudo -u postgres createdb marmitex
sudo -u postgres psql -c "CREATE USER marmitex WITH PASSWORD 'senha_segura';"
sudo -u postgres psql -c "ALTER ROLE marmitex WITH CREATEDB;"

# Configure .env
cat > .env << EOF
DATABASE_URL=postgresql://marmitex:senha_segura@localhost:5432/marmitex
...
cat > .env << EOF
DATABASE_URL=postgresql://marmitex:senha_segura@postgres:5432/marmitex
...
services:
  ...
  backend:
    image: seu-usuario/marmitex-system:latest
    ports:
      - "3001:3001"
    environment:
      DATABASE_URL: postgresql://marmitex:${DB_PASSWORD}@postgres:5432/marmitex
      REDIS_URL: redis://redis:6379
      NODE_ENV: production
