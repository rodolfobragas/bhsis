# Guia de Instalação - Marmitex System
...
export DATABASE_URL="postgresql://seu_usuario:sua_senha@localhost:5432/marmitex"
```
...
docker run --name marmitex-postgres \
  -e POSTGRES_USER=marmitex \
  -e POSTGRES_PASSWORD=marmitex_password \
  -e POSTGRES_DB=marmitex \
  -p 5432:5432 \
  -d postgres:15-alpine

# Configure a URL
export DATABASE_URL="postgresql://marmitex:marmitex_password@localhost:5432/marmitex"
```
...
docker run --name marmitex-redis \
  -p 6379:6379 \
  -d redis:7-alpine
...
```env
DATABASE_URL=postgresql://marmitex:marmitex_password@localhost:5432/marmitex
...
```env
DATABASE_URL=postgresql://user:password@db-host:5432/marmitex
...
npm install -g pm2
pm2 start dist/index.js --name "marmitex-api"
